...
Write to `docs/reviews/{user}-{branch}-eng-review-test-plan-{datetime}.md`:
...
```bash
mkdir -p docs/reviews
echo '{"skill":"plan-eng-review","timestamp":"TIMESTAMP","status":"STATUS","unresolved":N,"critical_gaps":N,"issues_found":N,"mode":"MODE","commit":"COMMIT"}' >> docs/reviews/review-log.jsonl
```
...
```bash
mkdir -p docs/reviews
echo '{"skill":"plan-eng-review","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}' >> docs/reviews/learnings.jsonl
```
